...
### 构建 (APK 修改方案)
```bash
cd android/patch/ && bash patch_and_build.sh ../miclaw-apks/魔改版带sese.apk
```
修改 miclaw APK 本身，不改包名，只改品牌/API/语音路由。
需要: apktool, apksigner, zipalign, keytool, Java 17+

### 云端部署 (母体 Token 池)
```bash
cd cloud/mother-body/ && bash scripts/deploy.sh
```
需要: Rust 1.77+, SQLite, Nginx

### miclaw 文件位置（香港服务器容器内）
- /workspace/project/MBclaw-Lite/miclaw-apks/ (1.5GB) — 5 个 APK + 4 个语音 APK
- /workspace/project/MBclaw-Lite/miclaw-decompiled/ (2.1GB) — cloud + sese 反编译
